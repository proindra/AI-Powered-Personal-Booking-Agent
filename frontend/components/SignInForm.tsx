...
export default function SignInForm() {
  ...
  const handleGoogleClick = () => {
    if (!CLIENT_ID) { setError('Google sign-in is not configured.'); return; }
      ...
      <form className="flex flex-col gap-5" onSubmit={handleSubmit}>
        ...
        <div className="flex flex-col gap-2">
          <div className="flex justify-between items-center">
            <label htmlFor="password" className="text-[0.65rem] font-bold tracking-[0.3em] text-white/50 uppercase">Password</label>
            <a href="#" className="text-[0.65rem] font-bold tracking-[0.1em] text-brand hover:text-white transition-colors uppercase">Forgot?</a>
          </div>
          <input type="password" id="password" name="password" required placeholder="••••••••"
            className="bg-white/5 border border-white/10 focus:border-brand w-full px-4 py-3 text-sm font-medium text-white placeholder-white/20 outline-none transition-all focus:bg-brand/5 focus:shadow-[0_0_20px_rgba(255,95,31,0.15)]" />
        </div>
        <button type="submit" className="mt-1 w-full py-4 px-8 text-sm font-black uppercase tracking-widest transition-all duration-300 bg-white text-black hover:bg-brand hover:text-white">
          Sign In
        </button>
      </form>
      ...
      <div className="mt-5 flex flex-col gap-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <button type="button" onClick={handleGoogleClick} disabled={loading}
            className="border border-white/10 hover:border-white/30 py-3 flex items-center justify-center gap-2 hover:bg-white/5 transition-all disabled:opacity-50">
            {loading
